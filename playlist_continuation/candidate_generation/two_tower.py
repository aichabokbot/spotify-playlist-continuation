...
from playlist_continuation.candidate_generation import train_utils
import playlist_continuation.candidate_generation.config as cfg


MAX_PLAYLIST_LENGTH = cfg.TRACK_HISTORY
...
class Playlist_Model(tf.keras.Model):
    ...
    def __init__(
        ...
        self.num_pl_songs_new_embedding = tf.keras.Sequential(
            [
                tf.keras.layers.Discretization(
                    train_utils.get_buckets_20(376)
                ),  # TODO - 376 from TRAIN
                tf.keras.layers.Embedding(
                    input_dim=20 + 1,
                    output_dim=embedding_dim,
        ...
        self.num_pl_artists_new_embedding = tf.keras.Sequential(
            [
                tf.keras.layers.Discretization(
                    train_utils.get_buckets_20(238)
                ),  # TODO - 238 from TRAIN
                tf.keras.layers.Embedding(
                    input_dim=20 + 1,
                    output_dim=embedding_dim,
        ...
        self.num_pl_albums_new_embedding = tf.keras.Sequential(
            [
                tf.keras.layers.Discretization(
                    train_utils.get_buckets_20(244)
                ),  # TODO - 244 from TRAIN
                tf.keras.layers.Embedding(
                    input_dim=20 + 1,
                    output_dim=embedding_dim,
        ...
        self.album_name_pl_embedding = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.GlobalAveragePooling2D(
                    name="album_name_pl_emb_layer_2d"
                ),
            ],
        ...
        self.duration_ms_songs_pl_embedding = tf.keras.Sequential(
            [
                tf.keras.layers.Discretization(
                    train_utils.get_buckets_20(20744575)
                ),  # 20744575.0
                tf.keras.layers.Embedding(
                    input_dim=20 + 1,
                    output_dim=embedding_dim,
                ...
                tf.keras.layers.GlobalAveragePooling1D(
                    name="duration_ms_songs_pl_emb_layer_pl_1d"
                ),
            ],
        ...
        self.artists_followers_pl_embedding = tf.keras.Sequential(
            [
                tf.keras.layers.Discretization(
                    train_utils.get_buckets_20(94437255)
                ),  # TODO - was 100
                tf.keras.layers.Embedding(
                    input_dim=20 + 1,
                    output_dim=embedding_dim,
        ...
        self.track_instrumentalness_pl_embedding = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.GlobalAveragePooling1D(
                    name="track_instrumentalness_pl_1d"
                ),
            ],
    ...
    def call(self, data):
        ...
        all_embs = tf.concat(
            [
                ...
                self.track_name_pl_embedding(
                    tf.reshape(data["track_name_pl"], [-1, MAX_PLAYLIST_LENGTH, 1])
                ),
                self.artist_uri_pl_embedding(data["artist_uri_pl"]),
                self.artist_name_pl_embedding(
                    tf.reshape(data["artist_name_pl"], [-1, MAX_PLAYLIST_LENGTH, 1])
                ),
                self.album_uri_pl_embedding(data["album_uri_pl"]),
                self.album_name_pl_embedding(
                    tf.reshape(data["album_name_pl"], [-1, MAX_PLAYLIST_LENGTH, 1])
                ),
                self.artist_genres_pl_embedding(
                    tf.reshape(data["artist_genres_pl"], [-1, MAX_PLAYLIST_LENGTH, 1])
                ),
                ...
                self.artists_followers_pl_embedding(data["artist_followers_pl"]),
                ...
                self.track_instrumentalness_pl_embedding(
                    data["track_instrumentalness_pl"]
                ),
...
class Candidate_Track_Model(tf.keras.Model):
    def __init__(
        ...
        self.track_uri_can_embedding = tf.keras.Sequential(
            [
                tf.keras.layers.Hashing(num_bins=2249561),
                tf.keras.layers.Embedding(
                    input_dim=2249561 + 1,
                    output_dim=embedding_dim,
        ...
        self.artists_followers_can_embedding = tf.keras.Sequential(
            [
                tf.keras.layers.Discretization(train_utils.get_buckets_20(94437255)),
                tf.keras.layers.Embedding(
                    input_dim=20 + 1,
                    output_dim=embedding_dim,
        ...
        self.track_loudness_can_embedding = tf.keras.Sequential(
            [
                tf.keras.layers.Discretization(train_utils.get_buckets_20(5)),
                tf.keras.layers.Embedding(
                    input_dim=20 + 1,
                    output_dim=embedding_dim,
        ...
        self.track_tempo_can_embedding = tf.keras.Sequential(
            [
                tf.keras.layers.Discretization(
                    train_utils.get_buckets_20(250)
                ),  # TODO - was 100
                tf.keras.layers.Embedding(
                    input_dim=20 + 1,
                    output_dim=embedding_dim,
    ...
    def call(self, data):
        all_embs = tf.concat(
            [
                ...
                self.track_instrumentalness_can_embedding(
                    data["track_instrumentalness_can"]
                ),
        ...
        self.task = tfrs.tasks.Retrieval(
            ...
            batch_metrics=[
                tf.keras.metrics.TopKCategoricalAccuracy(
                    10, name="batch_categorical_accuracy_at_10"
                ),
                tf.keras.metrics.TopKCategoricalAccuracy(
                    50, name="batch_categorical_accuracy_at_50"
                ),
            ],
